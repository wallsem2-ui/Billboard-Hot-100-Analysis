...
billboard <- billboard %>%
  ...
  filter(year>1999)%>%
  select(c(song, artist, year, weeks_at_number_one, overall_rating, artist_place_of_origin, front_person_age, length_sec, bpm, cdr_genre, happiness))%>%
  mutate(song = unlist(song))%>%
  drop_na(weeks_at_number_one, length_sec, bpm, overall_rating)

#initial exploration needed (dimensions, structure, variable types)
```
